...
check_libc() {
	...
	if echo "$MAJ_VER $MAX_SUPPORTED_MAJ_VER $MIN_VER $MAX_SUPPORTED_MIN_VER" | awk '{exit $1>$2||$3>$4?0:1}'; then
		echo "This package of NetViz agent requires minimum $VER version of the GLIBC on this system ($MAX_SUPPORTED_VER)."
		fail_installation
	fi
}
...
print_mount_error() {
	echo "Filesystem where NetViz agent is getting installed should not have 'noexec' or 'nosuid' flags enabled."
	echo "Use 'mount' command to see more details."
	log_msg "Mount point $1 has noexec or nosuid flag set"
}
...
do_perm() {
	...
	if which setcap > /dev/null 2>&1; then
		log_msg "Setting capabilities on $file"
		chmod u=rwx,g=rx,o=rx $file
		if ! setcap cap_net_raw,cap_net_admin,cap_net_bind_service=eip $file > /dev/null 2>&1; then
			log_msg "Setting capabilities for $file failed."
			log_msg "Falling back to setting set-user-id bit."
			chown root $file
			chmod u=rwxs,g=rx,o=rx $file
		fi
	else
		log_msg "Setting set-user-id on $file"
		chown root $file
		chmod u=rwxs,g=rx,o=rx $file
	fi
}
...
do_config_change() {
	sed -i -e "s:ROOT_DIR=.*:ROOT_DIR=\"$NPM_ROOT_DIR\":g" $AGENT_CONF_FILE
}
