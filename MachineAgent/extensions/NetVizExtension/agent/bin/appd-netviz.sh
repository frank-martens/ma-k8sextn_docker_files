...
NETMON_LOG=${NPM_ROOT_DIR}/logs/appd-netmon.log.0
NETAGENT_LOG=${NPM_ROOT_DIR}/logs/appd-netagent.log.0
...
has_permissions() {
	...
	if [ -u $AGENT_BIN ] ; then
		echo "info: running with setuid set for the agent"
		return 1
	fi
	...
	if [ $? -ne 0 ]; then
		echo "error: capabilities for capture are not set for the agent"
		return 0
	fi
	echo "info: running with capabilities set for the agent"
	return 1
}
...
do_start()
{
	...
	if [ $? -eq 0 ]; then
		echo "Failed"
		echo "Check ${NETMON_LOG} and ${NETAGENT_LOG} for more details"
	...
	is_npm_running
	if [ $? -eq 1 ]; then
		echo "${DESC} ${PKG_NAME} already running"
		exit $AGENT_ALREADY_RUNNING
	fi

	cleanup

	echo "Starting ${DESC} ${PKG_NAME}..."
	/bin/bash -c "$DAEMON $DAEMON_OPTS"
	if [ "$?" -eq 0 ]; then
		sleep 5
		is_npm_running
		if [ $? -eq 1 ]; then
			echo "${PKG_NAME} running.. ${pid}"
		else
			echo "Failed"
			echo "Check ${NETMON_LOG} and ${NETAGENT_LOG} for more details"
...
do_stop()
{
	...
	is_npm_running
	if [ $? -eq 0 ]; then
		echo "${DESC} ${PKG_NAME} is not running"
...
do_status()
{
	if is_npm_running == 0; then
		echo "${DESC} ${PKG_NAME} is stopped"
	else
		echo "${DESC} ${PKG_NAME} is running"
	fi
}
